...
import { hero, metrics, projects, services, stack } from "@/app/data/site-content";

type ContactActionProps = {
  onOpenContact: () => void;
};

export function TopNav({ onOpenContact }: ContactActionProps) {
  const navItems = [
    { href: "#services", label: "01. Services" },
    { href: "#projects", label: "02. Projects" },
    { href: "#tech-stack", label: "03. Stack" },
    { href: "#edge", label: "04. Edge" },
  ];
...
export function HeroSection({ onOpenContact }: ContactActionProps) {
  return (
    <section className="section-shell relative flex min-h-[870px] items-center overflow-hidden pb-20 pt-32">
      ...
      <div className="relative z-10 mx-auto max-w-5xl px-6">
        ...
        <h1 className="mb-8 text-4xl font-black tracking-tight leading-[1.1] md:text-6xl lg:text-7xl">
          {hero.title.split("replace manual work")[0]}
          <span className="gradient-text">replace manual work</span>
          {hero.title.split("replace manual work")[1]}
        </h1>
        <p className="mb-12 max-w-3xl text-lg leading-relaxed text-[var(--muted)] md:text-xl">{hero.description}</p>
        <button
          type="button"
          onClick={onOpenContact}
...
export function ProofSection() {
  ...
  return (
    <section className="overflow-hidden border-y border-white/10 bg-[#091126] px-6 py-24">
      <div className="mx-auto max-w-4xl">
        <div className="mb-12">
          <span className="mb-2 block font-mono text-[10px] font-bold uppercase tracking-[0.18em] text-[var(--primary)]">
            [ impact_metrics ]
          </span>
          <h2 className="text-3xl font-bold tracking-tight">Proof of Performance</h2>
        </div>
...
export function PhilosophySection() {
  ...
  return (
    <section className="border-y border-white/10 bg-[#091126] px-6 py-16">
      <div className="mx-auto max-w-5xl">
        <div className="flex flex-col items-start gap-8 py-10 md:flex-row md:items-center md:gap-10">
          <div className="group relative">
            ...
            <div className="absolute -bottom-2 -right-2 flex h-8 w-8 items-center justify-center rounded-full border-4 border-[#091126] bg-[var(--primary)] text-[11px] font-bold text-[#0c1528] shadow-[0_0_20px_rgba(152,203,255,0.35)] transition-shadow group-hover:shadow-[0_0_28px_rgba(152,203,255,0.55)]">
              ✓
            </div>
          ...
          <div className="max-w-3xl">
            ...
            <h2 className="mt-2 text-5xl leading-none text-[#d7e0fb]" style={{ fontFamily: "cursive" }}>
              Yevgen Galamaga
            </h2>
            <p className="text-[clamp(1.6rem,2.2vw,2.15rem)] italic leading-[1.45] tracking-[-0.01em] text-[#b9c3d8]">
              {philosophyQuote}
            </p>
          </div>
        </div>
    ...
    <section id="projects" className="bg-[var(--surface-low)] px-6 py-24">
      <div className="mx-auto max-w-6xl space-y-10">
        <div className="mb-16">
          ...
          <h2 className="text-4xl font-bold tracking-tight">Signature Projects</h2>
        </div>
  ...
  const edgeItems = [
    {
      ...
      quote: "I don't just build apps - I automate entire workflows",
      color: "text-[var(--primary)]",
    },
    {
      id: "02",
      title: "ROI-Driven Dev",
      quote: "I focus on ROI, not just code",
      color: "text-[var(--secondary)]",
    },
    {
      id: "03",
      title: "Hybrid Expertise",
      quote: "I combine AI + backend + automation in one system",
      color: "text-[var(--tertiary)]",
    },
    {
      id: "04",
      title: "Architectural Logic",
      quote: "I think in systems, not features",
    ...
    <section id="edge" className="px-6 py-24">
      <div className="mx-auto max-w-5xl">
        <div className="mb-16 text-center">
          ...
          <h2 className="text-4xl font-bold tracking-tight">The Systems Thinking Advantage</h2>
        </div>
    ...
    <section id="tech-stack" className="bg-[var(--surface-low)] px-6 py-24">
      <div className="mx-auto max-w-6xl">
        <div className="mb-16 text-center">
          ...
          <h2 className="text-4xl font-bold tracking-tight">The Tech Stack</h2>
        </div>
        <div className="grid gap-6 md:grid-cols-3">
          <StackColumn title="AUTOMATION_AI" values={stack.automation} color="var(--secondary)" />
          <StackColumn title="FRONTEND_CORE" values={stack.frontend} color="var(--primary)" />
          <StackColumn title="BACKEND_SYSTEM" values={stack.backend} color="var(--tertiary)" />
        </div>
      </div>
    </section>
  );
...
export function CtaBanner({ onOpenContact }: ContactActionProps) {
  return (
    <section className="bg-[#0a1020] px-6 py-28 text-center">
      <p className="mb-8 font-mono text-sm font-bold uppercase italic tracking-tight text-[var(--primary)] opacity-80">
        &quot;IF_TASK_REPEATS == TRUE {`{`} AUTOMATE(); {`}`}&quot;
      </p>
      ...
      >
        <span>Tell me what you want to eliminate — I&apos;ll build the system</span>
...
export function SiteFooter() {
  return (
    <footer className="border-t border-white/10 bg-[#070e1f] px-8 py-16">
      <div className="mx-auto flex max-w-6xl flex-col items-start gap-8 md:flex-row md:items-center md:justify-between">
        <div className="space-y-3">
          <p className="font-mono text-xl font-bold tracking-tight text-[#dce4fb]">YEVGEN_GALAMAGA</p>
          <p className="font-mono text-[10px] font-bold uppercase tracking-[0.2em] text-[#8f98aa]">
            © 2024 Kinetic Architect of Automation.
          </p>
        </div>
